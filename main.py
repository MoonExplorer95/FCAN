...
def get_parser():
    ...
    parser.add_argument('--run_peer', type=str, default=True)
    ...
    # transfer related
    parser.add_argument('--transfer_loss_weight', type=float, default=0.1) #default=10)
...
def train(source_loader, target_train_loader, target_test_loader, model, optimizer, lr_scheduler, args):
    ...
    path = './record/' + args.src_domain + '->' + args.tgt_domain + '/' + os.path.dirname(args.config) + '/' 
    ...
    for e in range(1, args.n_epoch+1):
        ...
        train_loss_aux = utils.AverageMeter()
        ...
        for _ in range(n_batch):
            ...
            clf_loss, transfer_loss, aux_loss = model(data_source, data_target, label_source)
            # loss = clf_loss + args.transfer_loss_weight * transfer_loss + args.transfer_loss_weight * aux_loss
            loss = clf_loss + 0.5 * transfer_loss + 1.0 * aux_loss
            ...
            train_loss_aux.update(aux_loss.item())
            train_loss_total.update(loss.item())
            
        log.append([train_loss_clf.avg, train_loss_transfer.avg, train_loss_aux.avg, train_loss_total.avg])
        
        info = 'Epoch: [{:2d}/{}], cls_loss: {:.4f}, transfer_loss: {:.4f}, aux_loss: {:.4f}, total_Loss: {:.4f}'.format(
                        e, args.n_epoch, train_loss_clf.avg, train_loss_transfer.avg, train_loss_aux.avg, train_loss_total.avg)
        # Test
        stop += 1
